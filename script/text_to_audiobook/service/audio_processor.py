...
class AudioProcessor:
    ...
    def process_audio_pipeline(self, sentence_files: List[str], output_dir: str, voice: str = "af_bella", speed: float = 1.0, compress: bool = True) -> Tuple[List[str], List[str]]:
        """
        完整的音频处理流程：生成 + 压缩
        ...
        """
        # 1. 生成音频
        audio_files, subtitle_files = self.generate_audio_files(sentence_files, output_dir, voice, speed)
        
        # 2. 压缩音频（如果需要）
        if compress and audio_files:
            self.compress_audio_files(audio_files, output_dir)
        
        return audio_files, subtitle_files
    
    def get_existing_audio_files(self, output_dir: str) -> List[str]:
        """
        获取已存在的音频文件
        ...
        """
        audio_dir = os.path.join(output_dir, OUTPUT_DIRECTORIES['audio'])
        return self.file_manager.get_files_by_extension(audio_dir, ".wav")
    
    def get_existing_subtitle_files(self, output_dir: str) -> List[str]:
        """
        获取已存在的字幕文件
        ...
        """
        subtitle_dir = os.path.join(output_dir, OUTPUT_DIRECTORIES['subtitles'])
        return self.file_manager.get_files_by_extension(subtitle_dir, ".srt")